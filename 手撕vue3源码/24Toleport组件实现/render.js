...
function queuePendingPostJob() {
  for (let i = 0; i < pendingPostFlushCbs.length; i++) {
    pendingPostFlushCbs[i] && pendingPostFlushCbs[i]();
  }
}
...
function createRender(options) {
  ...
  function patch(oldVnode, newVnode, container, anchor = null) {
    ...
    } else if (typeof type === "object" && newVnode.type.__isTeleport) {
      type.process(oldVnode, newVnode, container, {
        patch,
        patchChildren,
        insert,
        getElementSelector,
      ...
      () => {
        ...
        if (!instance.isMounted) {
          ...
          if (instance.mounted.length > 0) {
            pendingPostFlushCbs.push(...instance.mounted);
          }
        } else {
...
// 实现思路就是teleport组件不走mounteComponet
// 在patch中判断如果是teleport组件的话取出它的props中的to属性然后querselector拿到el几点然后把它的子节点一次挂载到el节点上面去
const Teleport = {
  __isTeleport: true,
  process(oldVnode, newVnode, container, context) {
    const { patch, getElementSelector } = context;
    if (!oldVnode) {
      const target =
        typeof newVnode.props.to === "string"
          ? getElementSelector(newVnode.props.to)
          : newVnode.props.to;
      console.log(target, newVnode.props.to);
      newVnode.child.forEach((vnode) => patch(null, vnode, target));
    }
  },
};
const com1 = {
  setup() {
    onMounted(() => {
      console.log(document.querySelector("#com"));
    });
    return () => {
      return {
        type: "div",
