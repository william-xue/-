...
function createRender(options) {
  ...
  function patchChildren(oldVnode, newVnode, container) {
    ...
    if (typeof newVnode.child === "string") {
      ...
      if (Array.isArray(oldVnode.child)) {
        // 简单diff算法
        const oldChild = oldVnode.child;
        const newChild = newVnode.child;
        // 用来记录老的vnode中最大的索引
        let lastIndex = 0;
        for (let i = 0; i < newChild.length; i++) {
          // 遍历新的vnode 看在老的vnode节点中是否存在
          const newVnodeC = newChild[i];
          // 标识在老节点中是否找到新节点
          let find = false;
          let anchor;
          for (let j = 0; j < oldChild.length; j++) {
            const oldVnodeC = oldChild[j];
            if (newVnodeC.key === oldVnodeC.key) {
              find = true;
              // 将老节点的值更新为新节点的值
              patch(oldVnodeC, newVnodeC, container);
              // 移动节点
              if (j < lastIndex) {
                // 如果当前老的vnode索引小于最大索引说明当前的真实dom需要移动到上一个节点的后面
                const preVnodeC = newChild[i - 1];
                if (preVnodeC) {
                  anchor = preVnodeC._el.nextSibling;
                  insert(newVnodeC._el, container, anchor);
                }
              } else {
                lastIndex = j;
              }
          ...
          if (!find) {
            ...
            patch(null, newVnodeC, container, anchor);
          }
        }
        // 遍历老节点 看老节点在新节点中是否存在 如果不存在需要卸载掉
        for (let i = 0; i < oldChild.length; i++) {
          const oldVnodeC = oldChild[i];
          const res = newChild.find((newC) => newC.key === oldVnodeC.key);
          console.log(res);
          if (!res) {
            unmount(oldVnodeC);
          }
