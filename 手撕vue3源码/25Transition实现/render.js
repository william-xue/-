...
function createRender(options) {
  ...
  function mountComponent(newVnode, container, anchor) {
    // 挂载组件阶段
    const { needTransition, transitionHook } = newVnode;
    ...
    effect(
      () => {
        ...
        if (needTransition) {
          ...
          vnode.needTransition = needTransition;
        }
        // 如何渲染到页面中可以通过patch渲染
  ...
  function mountElement(vnode, container, anchor) {
    ...
    if (vnode.needTransition) {
      vnode.transitionHook.beforeEnter(el);
    }
    insert(el, container, anchor);
    if (vnode.needTransition) {
      vnode.transitionHook.enter(el);
    }
  }
  ...
  function unmount(vnode) {
    ...
    if (el) {
      ...
      if (parent) {
        if (vnode.needTransition) {
          vnode.transitionHook.leave(el, remove);
        } else {
          remove();
        }
      }
...
// 实现原理transition.html Transition的组件主要是给组件增强一些钩子函数
const nextForm = (callback) => {
...
const Transition = {
  setup(props, { slot }) {
    ...
    rawVnode.needTransition = true;
    rawVnode.transitionHook = {
